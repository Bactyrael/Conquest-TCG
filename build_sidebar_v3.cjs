...
// Use regex to strictly replace the starting tags of the final return block
content = content.replace(/  return \(\s*<Xwrapper>\s*<div className="game-board">/, layoutStart);

// Remove the old zoom modal
content = content.replace(/\{\/\* Zoom Modal \*\/\}[\s\S]*?\{\/\* Annotation Modal \*\/\}/, "{/* Annotation Modal */}");

// Safely replace the end of the file using a regex to match the EXACT old closing sequence
const oldEndRegex = /    <\/div>\s*\{arrows\.map\(arrow => \(\s*<Xarrow[^>]+>\s*\)\)\}\s*<\/div>\s*<\/Xwrapper>\s*\);\s*\}/;
...
content = content.replace(oldEndRegex, layoutEnd);
